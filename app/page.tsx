...
export default function LighthousePage() {
  ...
  return (
    <div className="relative h-screen w-full bg-black text-white font-sans overflow-hidden selection:bg-white/20">
      ...
      {/* ANA İÇERİK - PT-24 YAPILDI (Eskisi 32/44) */}
      <main className={`relative z-10 w-full h-full pt-24 lg:pt-32 flex justify-center transition-all duration-700 ${isExpanded || isWriting || showSetup || isDailyOpen || isMissionModalOpen || isMissionListOpen || selectedMission ? 'scale-90 opacity-0 pointer-events-none blur-xl' : 'scale-100 opacity-100'}`}>
        <div className="w-full max-w-2xl px-4 flex flex-col items-center mx-auto">
            {!isLoading && dailyQuestion && (
                <motion.button initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="relative z-40 w-full max-w-xl mb-2 flex items-center justify-between p-2.5 px-4 rounded-full border bg-black/40 backdrop-blur-xl border-blue-500/20 shadow-[0_0_15px_-5px_rgba(59,130,246,0.2)] cursor-pointer hover:border-blue-500/40 transition-all touch-action-manipulation text-left" onClick={() => { if(dailyQuestion) { fetchDailyResponses(dailyQuestion.id); setIsDailyOpen(true); } }}>
            ...
            {isLoading ? ( <div className="flex flex-col items-center gap-4 animate-pulse"><RefreshCw className="w-8 h-8 animate-spin text-zinc-500" /><span className="text-xs text-zinc-500 tracking-[0.3em]">SYNCHRONIZING...</span></div> ) : filteredSignals.length > 0 ? (
                viewMode === 'stack' ? (
                    <div className="relative w-full max-w-2xl h-[400px] md:h-[450px] lg:h-[550px] flex items-center justify-center perspective-1000 mt-2">
                        ...
                        {/* BUTONLAR YUKARI ALINDI (bottom-10) */}
                        <div className="absolute bottom-10 flex items-center gap-6 md:gap-12 z-30">
