...
import { 
  Radio, Signal, X, MessageSquare, Send, 
  ChevronLeft, ChevronRight, RefreshCw, User, CornerDownRight, ChevronDown, 
  LogIn, LogOut, Search, LayoutGrid, Layers, Plus, Reply, Sparkles, Globe, Briefcase, DollarSign, Calendar, Mail, Maximize2, AtSign, Video, Instagram, List, Grid, Podcast
} from "lucide-react";
import LavaBackground from '@/components/LavaBackground';
import ProfileSetup from '@/components/ProfileSetup';

// --- BASİT ÇEVİRİ SERVİSİ ---
...
export default function LighthousePage() {
  ...
  const [dailyResponses, setDailyResponses] = useState<any[]>([]);
  ...
  const [newMission, setNewMission] = useState({ 
      title: '', type: 'partner', description: '', budget: '', 
  ...
  const getFrequencyColor = (freqId: string) => {
    switch (freqId) {
        case 'help': return 'text-red-500 border-red-500/50';
        case 'dream': return 'text-purple-500 border-purple-500/50';
        case 'ai': return 'text-blue-500 border-blue-500/50';
  ...
  const fetchProfile = async (userId: string) => {
    ...
    if (!error && data) {
        setProfile(data);
        if (!data.is_setup_complete) setShowSetup(true);
    }
  };
    ...
    if (!error && data) {
        const enrichedData = await Promise.all(data.map(async (sig) => {
            ...
            return { 
                ...
                author_country: p?.country || sig.distance,
                author_occupation: p?.occupation || sig.role,
    ...
    const channel = supabase.channel('realtime-feed')
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'comments' }, (payload) => {
          if (profile && payload.new.reply_to === profile.username && payload.new.author !== profile.username) {
             const newNotif = { id: Date.now(), message: `Log update: @${payload.new.author} replied to you! 💬` };
      ...
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'likes' }, () => fetchSignals())
      .subscribe();
  ...
  const handleNewLikeNotification = async (likeData: any) => {
    ...
    if (signalOwner && signalOwner.author === profile?.username && likeData.user_id !== user?.id) {
        ...
        const newNotif = { id: Date.now(), message: `New signal from ${sender?.country || 'Unknown Sector'}! 📡` };
        setNotifications(prev => [newNotif, ...prev]);
  ...
  const handleBroadcast = async () => {
    ...
    const { error } = await supabase.from('signals').insert({
        ...
        author: profile?.username || user.user_metadata.full_name,
        role: profile?.occupation || 'Explorer', distance: profile?.country || 'Near Orbit'
    });
  ...
  const handleDailyResponse = async () => {
    ...
    const { error } = await supabase.from('signals').insert({
        content: dailyResponseText, translation: translated, frequency: 'general',
        author: profile?.username || user.user_metadata.full_name,
        role: profile?.occupation || 'Explorer', distance: profile?.country || 'Near Orbit',
  ...
  const handlePostComment = async (signalId: number) => {
      ...
      const { error } = await supabase.from('comments').insert({ 
          signal_id: signalId, content: commentText, author: profile?.username || user.user_metadata.full_name, reply_to: replyingTo
      });
      if (!error) { setCommentText(""); setReplyingTo(null); await fetchSignals(); setShowAllComments(true); 
          if (dailyQuestion && isDailyOpen) fetchDailyResponses(dailyQuestion.id);
      }
  ...
  const handleCreateMission = async () => {
      ...
      const contactInfoJSON = JSON.stringify({
          email: newMission.contact_email,
          skype: newMission.contact_skype,
          instagram: newMission.contact_insta
      });
      ...
      if (!error) {
          setNewMission({ title: '', type: 'partner', description: '', budget: '', contact_email: '', contact_skype: '', contact_insta: '' });
  ...
  const initiateReply = (username: string) => {
  ...
  const openDailyModal = () => {
      if (dailyQuestion) {
          fetchDailyResponses(dailyQuestion.id);
          setIsDailyOpen(true);
      }
  };
  ...
  const renderContactInfo = (contactInfoStr: string) => {
      try {
          ...
          return <div className="text-sm text-white font-mono select-all cursor-pointer break-all">{contactInfoStr}</div>;
      }
  };
  ...
  // YENİ: SWIPE İÇİN DRAG İŞLEYİCİ
  const handleDragEnd = (event: any, info: PanInfo) => {
      // 100px'den fazla kaydırılırsa kart değiştir
  ...
  const getCardStyle = (index: number) => {
    ...
    if (dist === 1) return { zIndex: 0, x: 350, scale: 0.8, opacity: 0.3, filter: "blur(4px)", display: "block" };
    if (dist === -1) return { zIndex: 0, x: -350, scale: 0.8, opacity: 0.3, filter: "blur(4px)", display: "block" };
    return { zIndex: 0, x: 0, opacity: 0, display: "none" };
  };

  const renderText = (signal: any) => {
      if ((isGlobalEnglish || translatedIDs[signal.id]) && signal.translation) {
    ...
    <div className="relative h-screen w-full bg-black text-white font-sans overflow-hidden selection:bg-white/20">
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 p-6 md:p-8 flex flex-col md:flex-row justify-between items-start transition-all duration-500 ${isWriting || showSetup ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}>
        <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-start">
            ...
            <div className="flex md:hidden items-center gap-3">
                 ...
                 {user ? (
                    <img src={user.user_metadata.avatar_url} className="w-8 h-8 rounded-full border border-white/20 cursor-pointer" onClick={() => router.push(`/profile/${encodeURIComponent(profile?.username || user.id)}`)} />
                 ) : (
                    <button onClick={() => supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/auth/callback` } })} className="p-2 bg-white text-black rounded-full"><LogIn className="w-4 h-4" /></button>
                 )}
            </div>
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
            ...
            {user ? (
                <div className="flex items-center gap-3 bg-white/5 border border-white/10 px-4 py-2 rounded-full backdrop-blur-md">
                    <button onClick={() => setIsGlobalEnglish(!isGlobalEnglish)} className={`p-1.5 rounded-full border transition-all ${isGlobalEnglish ? 'bg-white text-black border-white' : 'text-zinc-400 border-white/10 hover:text-white'}`} title="Global Translate">
      ...
      <main className={`relative z-10 w-full h-full pt-32 flex justify-center transition-all duration-700 ${isExpanded || isWriting || showSetup || isDailyOpen || isMissionModalOpen || isMissionListOpen || selectedMission ? 'scale-90 opacity-0 pointer-events-none blur-xl' : 'scale-100 opacity-100'}`}>
        ...
        <div className="w-full max-w-2xl px-4 flex flex-col items-center mx-auto">
            ...
            {/* DÜZELTME: Z-Index 40 ve Button olarak değiştirildi */}
            {!isLoading && dailyQuestion && (
                <motion.button 
                    initial={{ opacity: 0, y: -10 }} 
                    animate={{ opacity: 1, y: 0 }} 
                    className="relative z-40 w-full max-w-xl mb-4 flex items-center justify-between p-3 px-5 rounded-full border bg-black/40 backdrop-blur-xl border-blue-500/20 shadow-[0_0_15px_-5px_rgba(59,130,246,0.2)] cursor-pointer hover:border-blue-500/40 transition-all touch-action-manipulation text-left" 
                    onClick={openDailyModal}
                    onTap={openDailyModal}
                >
                ...
                viewMode === 'stack' ? (
                    <div className="relative w-full max-w-2xl h-[55vh] md:h-[550px] flex items-center justify-center perspective-1000 mt-4">
                        {filteredSignals.map((signal, index) => {
                            ...
                            const isTranslated = translatedIDs[signal.id]; 
                            return (
                                /* DÜZELTME: Drag Eklendi (Sürükleme) */
                                <motion.div 
                                    key={signal.id} 
                                    animate={style} 
                                    drag="x" 
                                        ...
                                        <div className="p-8 pb-4">
                                            <div className="flex items-center justify-between mb-8">
                                                <div className="flex items-center gap-3 group/author" onClick={(e) => { e.stopPropagation(); router.push(`/profile/${encodeURIComponent(signal.author)}`); }}>
                                                    <div className="w-10 h-10 rounded-full overflow-hidden border border-white/10 bg-white/5"><img src={signal.author_avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${signal.author}`} className="w-full h-full object-cover" alt="Avatar" /></div>
                                                    <div><div className="text-sm font-bold text-white group-hover/author:text-blue-400 transition-colors">{signal.author}</div><div className="text-[10px] text-zinc-500 uppercase tracking-widest">{signal.author_occupation} • {signal.author_country}</div></div>
                                                </div>
                                                <div className="flex items-center gap-3">
                                                    <button onClick={(e) => handleTranslate(e, signal)} className={`p-2 rounded-full border transition-all z-20 ${isTranslated ? 'bg-white text-black border-white' : 'bg-transparent text-zinc-500 border-white/10 hover:text-white'}`}><Globe className="w-3.5 h-3.5" /></button>
                                                    <button onClick={(e) => handleSendSignal(e, signal.id)} className="flex items-center gap-1.5 bg-white/5 hover:bg-white/10 px-2 py-1 rounded-md transition-all group/radar"><div className="relative flex items-center justify-center"><Radio className={`w-3.5 h-3.5 ${hasLiked ? 'text-blue-400' : 'text-zinc-500'}`} />{hasLiked && <span className="absolute inset-0 animate-ping bg-blue-400/40 rounded-full"></span>}</div><span className="text-[9px] font-bold text-zinc-400 group-hover/radar:text-white">{signal.likes?.length || 0}</span></button>
                                                    <div className={`text-[9px] font-bold px-2 py-1 rounded border ${getFrequencyColor(signal.frequency)}`}>{signal.frequency?.toUpperCase()}</div>
                                                </div>
                                            </div>
                                            <p className="text-xl md:text-2xl font-serif text-zinc-200 line-clamp-3 mt-4">"{renderText(signal)}"</p>
                                        </div>
                                            ...
                                            {signal.comments && signal.comments.length > 0 ? (
                                                signal.comments.slice(-2).map((comment: any) => (
                                                    <div key={comment.id} className="flex items-start gap-3 opacity-60">
                                                        ...
                                                        <div className="bg-white/5 rounded-2xl p-3 px-4 max-w-[90%]"><div className="text-[10px] text-zinc-500 font-bold mb-1">{comment.author}</div><p className="text-sm text-zinc-300 leading-relaxed line-clamp-1">{comment.content}</p></div>
                                                    </div>
                                                ))
                                            ) : <div className="flex items-center justify-center h-full opacity-30 gap-2 text-zinc-500 text-xs uppercase tracking-widest"><MessageSquare className="w-4 h-4" /><span>Static noise...</span></div>}
                                        </div>
                                    </div>
                                </motion.div>
                            );
                            ...
                            {filteredSignals.map((signal, index) => {
                                ...
                                return (
                                    <motion.div 
                                        ...
                                        initial={{ opacity: 0 }} 
                                        animate={{ opacity: 1 }} 
                                        className={`flex items-start gap-4 p-4 bg-white/[0.03] border border-white/10 rounded-2xl hover:bg-white/[0.07] transition-all group cursor-pointer`} 
                                        onClick={() => { setCurrentIndex(index); setIsExpanded(true); }}
                                    >
      ...
      {viewMode === 'log' && (
        <button
            ...
            className="fixed bottom-10 right-8 lg:right-auto lg:left-1/2 lg:-translate-x-1/2 z-50 w-14 h-14 md:w-16 md:h-16 bg-white text-black rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all"
        >
      ...
      <AnimatePresence>
        ...
        {/* ... (Diğer Modallar - Aynen Devam - Kodun tekrarını önlemek için sadece gerekli olanları yazdım, tüm modallar yukarıdaki gibi eklenmeli) ... */}
        {expandedBrief && (
        ...
        {/* ... (DİĞER MODALLAR BURADA OLACAK - Write / Expanded Signal vb. - Kodun tamamını kopyalarken unutmayın) ... */}
        {/* Expanded ve Write modalını da aşağıya ekliyorum tam olması için */}
        ...
        {isWriting && (
            <motion.div initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 100 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                <div className="w-full max-w-4xl bg-black/80 backdrop-blur-3xl border border-white/10 rounded-[3rem] shadow-2xl flex flex-col md:flex-row overflow-hidden relative">
                    <button onClick={() => setIsWriting(false)} className="absolute top-6 right-6 z-20 p-2 rounded-full bg-white/5 hover:bg-white/20 transition-colors"><X className="w-6 h-6 text-white" /></button>
                    <div className="w-full md:w-1/3 bg-white/5 border-r border-white/10 p-8 flex flex-col justify-between">
                        <div><h2 className="text-2xl font-serif mb-6">Select Frequency</h2><div className="space-y-3">{FREQUENCIES.filter(f => f.id !== 'all').map((freq) => (
                            <button key={freq.id} onClick={() => setSelectedFreq(freq)} className={`w-full flex items-center gap-3 p-4 rounded-2xl border transition-all ${selectedFreq.id === freq.id ? 'bg-white text-black border-white' : 'bg-transparent border-white/10 text-zinc-400 hover:bg-white/5'}`}><div className={`w-3 h-3 rounded-full ${freq.color}`} /> <span className="text-sm font-bold tracking-wide">{freq.name}</span></button>
                        ))}</div></div>
                    </div>
                    <div className="flex-1 p-8 md:p-12 flex flex-col relative">
                        ...
                        <div className="flex items-center justify-end pt-6 border-t border-white/5"><button onClick={handleBroadcast} disabled={isSending || !messageText.trim()} className="flex items-center gap-3 bg-white text-black px-8 py-4 rounded-full font-bold tracking-wide hover:scale-105 transition-transform disabled:opacity-50">{isSending ? <span>TRANSMITTING...</span> : <><span>BROADCAST</span><Send className="w-4 h-4" /></>}</button></div>
                    </div>
                </div>
            </motion.div>
        )}
        ...
        {isExpanded && !isWriting && currentSignal && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60" onClick={() => setIsExpanded(false)}>
                <motion.div initial={{ scale: 0.9, y: 50 }} animate={{ scale: 1, y: 0 }} className={`w-full max-w-3xl max-h-[90vh] bg-[#0a0a0a] border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col`} onClick={(e) => e.stopPropagation()}>
                    <div className="flex items-center justify-between p-6 border-b border-white/5 bg-white/[0.02] shrink-0">
                        <div className="flex items-center gap-3">
                            <div className="w-10 h-10 rounded-full border border-white/10 overflow-hidden bg-white/5"><img src={currentSignal.author_avatar || `https://api.dicebear.com/7.x/bottts/svg?seed=${currentSignal.author}`} className="w-full h-full object-cover" alt="Author" /></div>
                            <div><h2 className="font-bold text-white">{currentSignal.author}</h2><p className="text-xs text-zinc-500 uppercase">{currentSignal.frequency}</p></div>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-6 md:p-10 custom-scrollbar">
                        <p className="text-xl md:text-2xl font-serif text-zinc-200 leading-relaxed whitespace-pre-wrap mb-10 mt-6">"{renderText(currentSignal)}"</p>
                        <div className="pt-8 border-t border-white/5">
                            ...
                            <div className="space-y-6">
                                    ...
                                    <div key={comment.id} className="flex gap-4 group">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                ...
                                                {comment.reply_to && <span className="text-[10px] text-blue-400 bg-blue-500/10 px-1.5 py-0.5 rounded">Replying to @{comment.reply_to}</span>}
                                            </div>
                                ...
                                {!showAllComments && currentSignal.comments.length > 2 && (
                                    <button onClick={() => setShowAllComments(true)} className="flex items-center gap-2 text-xs text-blue-400 hover:text-blue-300 transition-colors mt-2 pl-12"><span>Expand {currentSignal.comments.length - 2} more logs</span><ChevronDown className="w-3 h-3" /></button>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-4 md:p-6 bg-black/40 border-t border-white/5 shrink-0">
                        ...
                        <div className="flex gap-3 items-center">
                            <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center shrink-0"><User className="w-5 h-5 text-zinc-500" /></div>
                            <div className="flex-1 relative">
                                <input ref={inputRef} type="text" value={commentText} onChange={(e) => setCommentText(e.target.value)} placeholder={replyingTo ? `Reply to @${replyingTo}...` : "Transmit a reply..."} className="w-full bg-white/5 border border-white/10 rounded-full py-3 px-5 text-sm text-white outline-none focus:border-white/30 pr-12" onKeyDown={(e) => e.key === 'Enter' && handlePostComment(currentSignal.id)} />
                                <button onClick={() => handlePostComment(currentSignal.id)} disabled={isPostingComment || !commentText.trim()} className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-white text-black rounded-full hover:scale-105 disabled:opacity-50 transition-all"><CornerDownRight className="w-4 h-4" /></button>
                            </div>
                        </div>
                    </div>
                </motion.div>
