...
// --- SENİN PAYLAŞTIĞIN SHADER KODLARI ---
const vertexShader = `
varying vec2 vUv;
uniform float time;
...
void main() {
    ...
    vec3 color = vec3(0.0); // Varsayılan renk siyah
    ...
    if (t > 0.0) {
        ...
        // ARKA PLAN RENGİ: Sitenin temasına uyması için SİYAH (0.0) veya ŞEFFAF (0.0 alpha) yaptık
...
function LavaLampShader() {
  ...
  // Ekran boyutu değişince shader'ı güncelle
  React.useEffect(() => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      // @ts-ignore - TS shader materyali bazen tanımaz
  ...
  return (
    <mesh ref={meshRef}>
      <planeGeometry args={[10, 10]} /> {/* Boyutu büyüttük ki ekranı tam kaplasın */}
      <shaderMaterial
        uniforms={uniforms}
        vertexShader={vertexShader}
        fragmentShader={fragmentShader}
        transparent={true} // Şeffaflık açık
      />
    </mesh>
  );
}

// Default export olarak dışarı açıyoruz
